...
RETURN c, round(avg(p.weight_g),2) AS Target_Weigth
...
WITH c, toFloat(count(se)) AS seller_number
...
RETURN s, round(avg(p.width_cm),2) AS AVG_width_cm, round(avg(p.height_cm),2) AS AVG_height_cm, round(avg(p.length_cm),2) AS AVG_length_cm, round(avg(p.weight_g),2) AS AVG_weight
